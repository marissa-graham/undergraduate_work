...
def train(names, samples):
    ...
    #print best_models
    for j in xrange(len(best_models)):
        print best_models[j]
    return best_models
...
def test(names, samples, models):
    results = []
    for name in names:
        for model in samples[name][20:]:
            best = -np.inf
            best_model = -1
            for j in xrange(len(models)):
                score = models[j].score(model)
                if score > best:
                    best = score
                    best_model = j
...
def run():
    ...
    print "Mathematics: 10/10, Biology: 6/10, Political Science: 10/10, Statistics: 10/10, Psychology: 10/10."
